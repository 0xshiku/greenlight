...
func (app *application) writeJSON(w http.ResponseWriter, status int, data envelope, headers http.Header) error {
	...
	//  Here we use no line prefix ("") and tab indents ("") and tab indents ("\t") for each element.
	...
	// At this point, we know that we won't encounter any more errors before writing the response, so it's safe to add any headers that we want to include.
	// We loop through the header map and add each header to the http.ResponseWriter header map.
	// Note that it's OK if the provided header map is nil. Go doesn't throw an error if you try to range over (or generally, read from) a nil map.
...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst interface{}) error {
	...
	// before decoding. This means that if the JSON from the client now includes any field which cannot be mapped
	// to the target destination, the decoder will return an error instead of just ignoring the field.
	...
	if err != nil {
		...
		// In some circumstance Decode() may also return an io.ErrUnexpectedEOF error
		...
		// Likewise, catch any *json.UnmarshalTypeError errors. These occur when the JSON value is the wrong type for the
		// target destination. If the error relates to a specific field, then we include that in our error message to make it
		...
		// We check for this errors.Is() and return a plain-english error message instead
		...
		// If the request body exceeds 1MB in size to decode will now fail with the error
		// "httpL request body too large".
		case err.Error() == "http: request body too larger":
			return fmt.Errorf("body must not be larger than %d bytes", maxBytes)

		// A json.InvalidUnmarshalError error will be returned if we pass a non-nil pointer to Decode().
		// We catch this an panic, rather than returning an error to our handler. At the end of this chapter we'll talk about
		// panicking versus returning errors, and discuss why it's an appropriate thing to do in this specific situation
	...
	// So if we get anything else, we know that there is additional data in the request body and we return
