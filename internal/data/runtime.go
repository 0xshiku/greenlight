...
// IMPORTANT: Because UnmarshalJSON() needs to modify the receiver (our Runtime type), we must use a pointer receiver for this to work correctly.
// Otherwise, we will only be modifying a copy (which is then discarded when this method returns).
func (r *Runtime) UnmarshalJSON(jsonValue []byte) error {
	// We expect that the incoming JSON value will be a string in the format "<runtime> mins", and the first thing we need to do is remove the surrounding
	// double-quotes from this string. If we can't unquote it, then we return the ErrInvalidRuntimeFormat error.
	...
	// Again, if this falls return the ErrInvalidRuntimeFormat error.
	...
	// Note that we use the * operator to deference the receiver (which is a pointer to a Runtime Type)
